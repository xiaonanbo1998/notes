...
3. 简单介绍：Regular Expression，描述文本内容组成规律的表示方式
   - :cherries:元字符：特殊的单独字符；空白符号；量词；范围
   - :cherries:模式：贪婪模式（默认模式）；非贪婪模式；独占模式
   - :cherries:匹配：不区分大小写匹配；单行匹配；多行匹配；注释匹配

4. 正则的常用方式

   ```javascript
   let regexp = /[A-E]/g		//	匹配大写的A-E字母
   let str = 'ABCDEFGabcdefg'
   let matches_array = str.match(regexp)	
   //	输出['A', 'B', 'C', 'D', 'E']
   
   let regexp = new RegExp(/[A-E]/g)	//	定义方式
   let str = 'ABCDEFGabcdefg'
   let is_matched = regexp.test(str)
