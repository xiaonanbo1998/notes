...
- [x] 统计表
  ...
  - 基础使用的三把斧
    ...
    > :book:配置项手册：[Documentation - Apache ECharts](https://echarts.apache.org/zh/option.html#title)
  
  - 优化方向
  
    1. 封装图表为组件，传入规则数据，进行渲染
    2. 添加页面大小变化的事件监听，同时添加防抖，以及监听的去除
    3. transition动画的添加
...
- [x] 量表开发
  ...
  - 问题点
    ...
    1. 怎么控制下滑：CSS3的transition属性，CSS3的var()函数，Less的变量，CSS2的overflow-y属性
    2. 吸顶组件长久存在：添加z-index属性
    3. 底部按钮针对苹果的适配：env(safe-area-inset-bottom)函数，CSS3的calc()函数
...
1. 医生端
   ...
   - "appid" : "__UNI__95CBC7A",
   - dev，13558716926，廖小小

2. 患者端

   - dev，庄周，340203199909097043

3. 健康调查表

   - 患者小程序
     - :heavy_check_mark:问卷部分，根据【0621】已经调整
     - :heavy_check_mark:结论部分，根据【0621】做相应调整，缺少【相应调整】文档
     - :heavy_check_mark:数据部分，没有根据【0621】相应调整/清理
     - :heavy_check_mark:本地版本已经更新，开发版本没有更新
   - 医生APP
     - :heavy_check_mark:问卷部分，根据【书情姐反馈】已经调整，并且根据【0621】做相应调整
     - :lemon:本地版本已经更新，其他部分都没有更新

4. 关于【症状自评量表】和【职业倾向量表】

   - 没有对应的产品设计
   - 【症状自评量表】优先完成，原定时间【0701/星期五】
   - 【职业倾向量表】低优先级，原定时间【0708/星期五】

5. 患者小程序和健康小屋小程序的打通

   - [x] 通过微信接口跳转

     - 患者小程序成功登录后，点击跳转，不用携带额外参数

     - 跳转健康小屋小程序之后，自动登录，或者手动授权再登录

       > :book:二维码过期问题：[开发版小程序已过期,请在开发者工具重新扫码 | 微信开放社区 (qq.com)](https://developers.weixin.qq.com/community/develop/doc/00088a2e6b42b0270f57c011356000)
   ...
   - [x] 小程序线上发布
     ...
     - 通过小工具的上传代码功能，然后在微信小程序后台审批即可

       > :book:发布流程：https://www.cnblogs.com/senyier/p/11065833.html
...
- [ ] 患者端更新相关【用户协议】和【隐私协议】
  ...
  - ```javascript
    //	涉及到个人服务器的相关链接等等，增加了加密和解密流程
    uni.navigateTo({
        url: '../web-view/web-view?url=' + encodeURIComponent(url) + '&type=' + type	//	加密
    })
    
    this.url = decodeURIComponent(opt.url)		//	解密
    ```
